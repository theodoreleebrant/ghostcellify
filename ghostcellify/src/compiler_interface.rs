...
pub type EarlyPass = dyn EarlyLintPass;
pub type LatePass = dyn for<'tcx> LateLintPass<'tcx>;
pub type EarlyCallback = fn() -> Box<EarlyPass>;
pub type LateCallback = for<'tcx> fn(TyCtxt<'tcx>) -> Box<dyn LateLintPass<'tcx> + 'tcx>;
...
pub fn run_compiler_with_setup<SetupFn: FnOnce(&mut DriverCallbacks)>(
    ...
    if cfg!(profile_compiler_runs) {
        log::debug!("rustc exited with code {}", exit_code);
        log::debug!("elapsed time: {} ms", compile_time.as_millis());
    }
